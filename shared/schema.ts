...
export const users = pgTable("users", {
  ...
  authProvider: varchar("auth_provider", { length: 20 }).default('replit'), // replit, google, local
  googleId: varchar("google_id"), // Google OAuth ID
  replitId: varchar("replit_id"), // Original Replit user ID
...
export const farmAnimals = pgTable("farm_animals", {
  ...
  weightUnit: varchar("weight_unit", { length: 10 }).default("lbs"),
  damId: integer("dam_id"), // mother reference - will add FK constraint later
  sireId: integer("sire_id"), // father reference - will add FK constraint later
...
export const animalTags = pgTable("animal_tags", {
  ...
  parentId: integer("parent_id").references((): any => animalTags.id), // for hierarchical relationships (breed -> species)
  description: text("description"),
